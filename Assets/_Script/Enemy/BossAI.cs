...
public class BossAI : MonoBehaviour
{
    ...
    public float detectionRadius = 8f; // Bán kính phát hiện player (lớn hơn enemy)
    public float chaseSpeed = 4f; // Tốc độ đuổi theo player (nhanh hơn enemy)
    ...
    public int maxHealth = 300; // Máu tối đa (cao hơn enemy)
    public int currentHealth; // Máu hiện tại
    public int meleeAttackDamage = 20; // Sát thương tấn công cận chiến (cao hơn enemy)
    public float meleeAttackRange = 1.5f; // Khoảng cách tấn công cận chiến
    public int rangedAttackDamage = 15; // Sát thương tấn công xa
    public float rangedAttackRange = 5f; // Khoảng cách tấn công xa
    public float attackCooldown = 2f; // Thời gian chờ giữa các đòn tấn công (dài hơn enemy)
    public GameObject deathEffect; // Hiệu ứng khi chết
    public GameObject projectilePrefab; // Prefab của đạn (cho tấn công xa)

    [Header("Avoidance Settings")]
    public float maxAvoidanceTime = 5f; // Thời gian tối đa để tránh chướng ngại vật

    [Header("Knockback Settings")]
    [SerializeField] private float KnockbackForce = 5f; // Lực đẩy lùi khi bị đánh (mạnh hơn enemy)
    [SerializeField] private float durPushBack = 0.3f; // Thời gian đẩy lùi
    ...
    private static readonly int MeleeAttackTrigger = Animator.StringToHash("MeleeAttack"); // ID animation tấn công cận chiến
    private static readonly int RangedAttackTrigger = Animator.StringToHash("RangedAttack"); // ID animation tấn công xa
    ...
    void MeleeAttack()
    {
        ...
        animator?.SetTrigger(MeleeAttackTrigger); // Kích hoạt animation tấn công cận chiến

        bool canAttack = player != null && Vector2.Distance(transform.position, player.position) <= meleeAttackRange;
        Vector2 knockbackDirection = (player.position - transform.position).normalized;
        StartCoroutine(DealMeleeDamageAfterDelay(0.3f, canAttack, knockbackDirection)); // Gây sát thương sau delay
    }

    IEnumerator DealMeleeDamageAfterDelay(float delay, bool canAttack, Vector2 knockbackDirection)
    {
        ...
        if (canAttack)
        {
            ...
            {
                playerController.TakeDamage(meleeAttackDamage, knockbackDirection); // Gây sát thương cận chiến
                AudioManager.Instance.PlayAttackSFX(); // Phát âm thanh tấn công
            }
        }
    ...
    void RangedAttack()
    {
        ...
        lastAttackTime = Time.time;
        rb.linearVelocity = Vector2.zero; // Dừng di chuyển khi tấn công
        animator?.SetTrigger(RangedAttackTrigger); // Kích hoạt animation tấn công xa

        StartCoroutine(FireProjectileAfterDelay(0.3f)); // Bắn đạn sau delay
    }
    ...
    IEnumerator FireProjectileAfterDelay(float delay)
    {
        ...
        if (projectilePrefab != null && player != null)
        {
            ...
            {
                projectileScript.SetDamage(rangedAttackDamage); // Gán sát thương cho đạn
                projectileScript.SetDirection(directionToPlayer); // Gán hướng cho đạn
            }
            AudioManager.Instance.PlayAttackSFX(); // Phát âm thanh tấn công
        }
        yield return new WaitForSeconds(0.5f);
    ...
    public void TakeDamage(int damage, Vector2 knockbackDirection)
    {
        ...
        CameraShake.ins.Shake(2, 30, 0.5f); // Rung camera mạnh hơn enemy
        ...
        else
        {
            ...
            AudioManager.Instance.PlayHitSFX(); // Phát âm thanh bị đánh
        }
    }
    ...
    void Die()
    {
        ...
        StartCoroutine(DestroyAfterDelay(1.5f)); // Xóa boss sau delay
    }
    ...
    void OnDrawGizmosSelected()
    {
        ...
        Gizmos.DrawWireSphere(transform.position, meleeAttackRange); // Vùng tấn công cận chiến
        Gizmos.color = Color.green;
        Gizmos.DrawWireSphere(transform.position, rangedAttackRange); // Vùng tấn công xa
        Gizmos.color = Color.yellow;
        Gizmos.DrawWireSphere(transform.position, patrolRadius); // Vùng tuần tra
    }
}